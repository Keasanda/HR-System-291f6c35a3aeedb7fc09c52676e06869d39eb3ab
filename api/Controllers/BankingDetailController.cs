...
[HttpGet("GetBankingDetails")]
public async Task<IActionResult> GetBankingDetails()
{
    // Get userID from the currently logged-in user (could be passed via JWT or token)
    ...
    // Fetch the corresponding banking details based on the AppUserId
    var bankingDetails = await _context.BankingDetails.FirstOrDefaultAsync(b => b.AppUserId == userId);
...
[HttpGet("{appUserId}")]
public async Task<IActionResult> GetBankingDetailsByAppUserId(string appUserId)
{
    // Fetch banking details by the AppUserId
    var bankingDetails = await _context.BankingDetails
        .Where(b => b.AppUserId == appUserId)
        .FirstOrDefaultAsync();

    // If no banking details are found, return a validation message
    if (bankingDetails == null)
    {
        return NotFound(new { message = "No banking details found for this user." });
    }
